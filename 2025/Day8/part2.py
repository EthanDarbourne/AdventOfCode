...
with open('input.txt', 'r') as file:
    ...
    for i in range(n):
        for j in range(i + 1, n):
            ...
            if len(closestPairs) > MAX_HEAP_SIZE:
                heapq.heappop(closestPairs)

    circuits = []
    closestPairs = [(-x[0], x[1], x[2]) for x in closestPairs]
    heapq.heapify(closestPairs)
    ...
    while len(closestPairs) > 0:
        ...
        longestCircuits = sorted([len(circuit) for circuit in circuits])
        if longestCircuits[0] == MAX_SIZE:
            print(box1[0] * box2[0], box1,box2)
            break
